...
tracker = BYTETracker(BYTETrackerArgs(), frame_rate=30)  # Điều chỉnh frame_rate nếu cần
previous_centroids = {}  # Lưu trữ centroid từ frame trước
...
def run(
    ...
    # Sử dụng global previous_centroids
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        detections_roi = []  # Thay rects bằng detections cho ByteTrack
        ...
        if len(pred):
            ...
            for *xyxy, conf, cls in reversed(pred):
                ...
                # Chỉ thêm detection trong ROI
                if ROI_X_MIN <= box_center_x <= ROI_X_MAX and ROI_Y_MIN <= box_center_y <= ROI_Y_MAX:
                    detections_roi.append([x1, y1, x2, y2, conf.item()])  # Bao gồm confidence
        ...
        # Xử lý tracking với ByteTrack
        current_centroids = {}
        if len(detections_roi) > 0:
            detections_roi_np = np.array(detections_roi)
        else:
            detections_roi_np = np.empty((0, 5))

        # Cập nhật tracker
        outputs = tracker.update(
            detections_roi_np,
            [im0.shape[0], im0.shape[1]],  # img_info: [height, width]
            [im0.shape[0], im0.shape[1]]   # img_size: [height, width] (scale=1)
        )
        ...
        for track in outputs:
            ...
            centroid_x = (x1 + x2) / 2
            centroid_y = (y1 + y2) / 2
            current_centroids[track_id] = (centroid_x, centroid_y)

            # Kiểm tra hướng di chuyển
            if track_id in previous_centroids:
                prev_x, prev_y = previous_centroids[track_id]
                direction = 'right' if centroid_y <= prev_y else 'wrong'
                color = (0, 255, 0) if direction == 'right' else (0, 0, 255)
                # Vẽ hướng và centroid
                cv2.putText(im0, direction, (int(centroid_x) - 10, int(centroid_y) - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                cv2.circle(im0, (int(centroid_x), int(centroid_y)), 3, color, -1)
            else:
                # Mới xuất hiện, vẽ màu mặc định
                color = (0, 255, 0)
                cv2.circle(im0, (int(centroid_x), int(centroid_y)), 3, color, -1)

        # Cập nhật previous_centroids cho frame tiếp theo
